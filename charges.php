...
<title>Employee Visitor Details</title>
...
<table border="0" align="center" bgcolor="#87CEFA" height="200">
  ...
  <tr>
    <td align="center">
	<form id="f1" action="insertcharges.php" id="registration" name="registration" method="post" onSubmit="return reg()">
	<table width="600" height="200">
	<tr>
	  <td align="center" bgcolor="#87CEFA" style="color:maroon" colspan="2">
	 
	Employee Visitor Details  </td>
	  </tr>
	<tr>
	<td width="110">Employee Visitor ID</td>
	<?php
	...
	{
	$Qry = mysql_query("SELECT ifnull(max(cid),0) as count FROM charge"); 
...
if(mysql_select_db("svisitor",$con))
	{
	$Qry = mysql_query("SELECT distinct eid  FROM extension"); 
	while($count = mysql_fetch_array($Qry)) 
 	{?>
		<option value="<?php echo $count['eid']; ?> "><?php echo $count['eid']; ?></option> 
	<?php }
...
<td>
  <textarea name="ddate" id="ddate" cols="45" rows="5"></textarea>
	...
	<tr>
	  ...
	  <?php if(@$_GET['status'])
	  {
	  	echo "Employee Visitor details Added";
	  }?>&nbsp;</td>
	  </tr>
	</table>
	</form>
